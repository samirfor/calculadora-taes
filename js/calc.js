...
function updateQuali (form, classs) {    
    var alloptions = Array("Nenhuma", "Fundamental Completo", "Médio Completo", "Médio Técnico/Profissionalizante", "Graduação Completa", "Especialização", "Mestrado", "Doutorado");
...
function valorTransporte(vencimento, gasto) {
    ...
    auxilio = gastodiario*22 - vencimento*0.06*(22/33);
    if (auxilio < 0) {
        return 0;
    } else {
        return auxilio;
...
function valorFG(FG, periodo) {
    ...
    var FG2015 = Array(0, 808.35, 543.82, 440.90, 224.41,  182.09, 133.54, 85.17, 62.99, 51.12);
    var valor = 0;
...
function calcSalario (form) {      
    ...
    var vencimento =  Math.round(base * (Math.pow(ftstep, ftvb)) * ftcarga * 100) / 100; 
    var alimentacao = 0;
    if (ftcarga == 0.5) {
    ...
    //var fttrans = (form.trans.checked) ? 1 : 0;
    var transporte = (form.trans.checked) ? valorTransporte(vencimento, form.gastoTrans.value) : 0;
    var ftinsa = (form.insa.checked) ? 0.1 : 0;
    //var ftcreche = (form.creche.checked) ? valorCreche(bruto) : 0;
    var ftpg = form.ddQuali.value;    
    var urp = vencimento*0.2605;
    var remuneracao = vencimento + urp + ftpg*(vencimento+urp) +  ftinsa*vencimento;    
    ...
    //var bruto = vencimento + urp + ftpg*(vencimento+urp) + ftalim*373 + fttrans*transporte + ftinsa*vencimento*0.1 + ftcreche*77; 
    var fungrat = valorFG(parseInt(form.ddFG.value, 10), periodo);
    //var fungrat = valorFG(0, 1);
    ...
    var baseirrf = baseinss + ftinsa*vencimento + fungrat - aliqinss;
    var aliqirrf = 0;
    if (baseirrf < 1710.79) {
        aliqirrf = 0;
    }  else if (baseirrf < 2563.92) {
        aliqirrf = baseirrf*0.075 - 128.31;
    } else if (baseirrf < 3418.60) {
        aliqirrf = baseirrf*0.15 - 320.60;
    } else if (baseirrf < 4271.59) {
        aliqirrf = baseirrf*0.225 - 577.00;
    } else {
        aliqirrf = baseirrf*0.275 - 790.58;
    }
    aliqirff = Math.floor(aliqirrf*100)/100;
    ...
    form.txInsa.value = formatValor(Math.ceil(ftinsa*vencimento*100)/100);
